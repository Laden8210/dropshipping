...
$data = $orderModel->initiateReturn($request_body['order_id']);
if ($data) {
    http_response_code(200);
    echo json_encode([
        'status' => 'success',
        'message' => 'Order marked as return',
        'orders' => $data
    ]);
} else {
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Failed to mark order as return']);
}