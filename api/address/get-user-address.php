...
$address = $addressModel->getAllAddresses($user_id);
if ($address) {
    http_response_code(200);

    foreach ($address as &$addr) {
        if (isset($addr['address_id'])) {
            $addr['address_id'] = (string)$addr['address_id'];
        }
    }
    unset($addr);
    echo json_encode(['status' => 'success', 'message' => 'Address retrieved successfully', 'data' => $address]);
} else {
